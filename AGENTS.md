...
## Simulation
`scripts/simulate.mjs` is the canonical balance check. It mirrors the live ship-part + crew-quarter-upgrade economy: 5 starting crew, 40-card Crew deck (10 unique blueprints with per-template copy counts tuned for exact icon balance — Juno/Priya ×5, Oren/Malik ×3, the rest ×4), hand size 5 with Balatro-style tired/Crew deck cycle within a sector, plus a full sector-end reshuffle (hand + tired + Crew deck are pooled, reshuffled, and a fresh hand of 5 is dealt after every gate clear; CQU-burned crew stay permanently gone), 3 mission actions per sector, fixed 10-gate ramp 8/9/16/17/21/24/27/31/34/38 (total 225, tuned so Sector 3 is a hard wall for no-joker play). Crew cards carry 1 or 2 specialization icons; single-icon cards cannot satisfy Specialist / Cross-Trained / Department Heads / Bridge Crew (those patterns require length-2 specializations) — they only contribute to shared-icon patterns. The 45-card roster is exactly icon-balanced at 16 each (E=L=N=S=16). Pattern fuel rewards: Cross-Trained 1, Common Ground 2, Specialist 2, Common Knowledge 3, Department Heads 4, Common Cause 4, Bridge Crew 6. Scrap reward tiers: 1-2 Fuel → 1 Scrap, 3-4 Fuel → 2, 5+ Fuel → 3. After each gate the player is offered 2 random ship parts from the 20-card un-owned research pool **plus** a one-button "Place Crew Quarters Upgrade" option (flat 4 Scraps, deals one generic Crew Quarters Upgrade card from an offscreen deck onto the board). Crew Quarters Upgrades are also dealable any time during a sector via a 4-Scrap stack action that triggers when the player accumulates exactly 4 Scraps in a single stack — incoming Scraps auto-stack onto the highest sub-4 pile so this happens naturally. When a CQU card is on the board, stacking 1-4 crew on it surfaces an `Upgrade <Pattern> (Lv N → M Fuel)` stack action: triggering it consumes the CQU card AND the crew used (the crew is permanently removed from the run, not pushed to tired), and grants +1 Fuel/play on the chosen pattern forever, up to **`CREW_QUARTERS_MAX_UPGRADES_PER_PATTERN` = 3 stacks per pattern** — past that the upgrade preview skips the maxed pattern (the action button vanishes when no satisfied pattern still has headroom). The cap prevents the mono-Common-Ground snowball that previously made one strategy dominate (35% mono win rate uncapped → ~13% with cap=3). Crew is then refilled from the Crew deck back up to the hand limit. The simulator's greedy player spends eagerly on CQU during sectors with a 4-Scrap buffer to keep ship-part purchases viable, then drains any leftover Scraps on additional upgrades in the post-gate dialog. A paid re-roll is available in the live dialog (ship parts only). Scraps come from missions and from joker triggers (Recovery Drone, Cargo Hold) — there is no bank-style interest and no skip consolation.

**Re-run `pnpm sim` whenever you touch:** crew rosters, hand patterns + their fuel rewards, gate costs, starting fuel, sector / action counts, ship part catalog, crew quarter upgrade catalog (including `CREW_QUARTERS_MAX_UPGRADES_PER_PATTERN`), or scrap economy. The simulator's data constants are mirrored at the top of the file with comments pointing at the source files (`src/game/blueprints/crewDecks.ts`, `src/game/rules.ts`, `src/game/blueprints/sectorGates.ts`, `src/game/economyTuning.ts`, `src/game/setup.ts`, `src/game/shipPartCatalog.ts`, `src/game/crewQuartersCatalog.ts`); update them in lockstep with the game.

**Strategy probes:** the sim accepts `STRATEGY=` env values for dominance audits. Useful probes: `mixed-greedy` (default, the "competent player" baseline), `upgrade-only-common-ground` (mimics the user playstyle that previously dominated at 35%), `upgrade-only-X` for any pattern (X = `common-knowledge` | `specialist` | `cross-trained` | `bridge-crew` | …), `broad-cheap` (only the easy patterns), `mono-X` (forces the play of one pattern). The healthy target is **no single strategy more than ~5× the win rate of mixed-greedy**; if a tuning change pushes mono-CG (or any other strategy) above that, the cap or rule needs rework. Override the cap with `PATTERN_LEVEL_CAP=999` to verify the dominance returns.

**Target metrics** for greedy max-fuel-pattern play (the strategy a competent player approximates):
- **Sector 1 pass rate ≈ 70%.** The opening hand is now drawn at random from the full 45-card deck (no privileged starter slot), so the player no longer gets a guaranteed Mara EE / Sana LL specialist double. A solid fraction of openers can't reach gate cost 8 and wash out at S1.
- **Sector 3 wall.** Without ship parts AND Crew Quarter Upgrades, ~0% of runs reach Sector 4 (S3 cost 16 vs greedy ~9 fuel/sector). The game design requires the player to start buying ship parts AND/OR researching Crew Quarter Upgrades to keep up.
- **Overall win rate ≈ 1.5–2.5%** with jokers ON. Roughly 1-in-50 greedy runs clears all 10 gates — strategic spend split between **5 Ship Parts** (Compounding Drive early, Crew Synergy mid, Veteran's Insignia / Adrenal Implants late) and **5-8 Crew Quarter Upgrades** (invest deeply in the cheap common patterns greedy plays most often — Common Ground, Cross-Trained, Common Knowledge) lifts this for skilled players.
- **Win rate = 0%** with jokers OFF (`NO_JOKERS=1`). The ramp is tuned so a no-joker, no-upgrade baseline cannot beat any gate from S3 onward.

**Target curve shape** (per-sector dropout, jokers ON):
- S1 ~28% (random opener can fail to reach gate cost 8), S2 ~14%, S3 ~43% (the wall — most runs die here without scaling parts/upgrades).
- S4 ~12%, S5 ~5%, S6 ~3%, S7 ~3% (steady decline as the back-end costs ramp).
- S8 ~3%, S9 ~2%, S10 ~1% (taper — only well-built joker + upgrade stacks survive the late gates).

**Target loadout (winning runs):**
- 5 Ship Parts filling the slot cap (typical: Lean Manifest, Reserve Capacitor, Preflight Tune-Up, Recovery Drone, Cargo Hold).
- ~10-16 Crew Quarter Upgrades researched. Distribution heavily skews toward the cheapest-crew patterns the greedy plays most: Cross-Trained ~5/win, Specialist ~4/win, Common Ground ~3.5/win, others ~0. With every upgrade flat-priced at 4 Scraps and granting +1 Fuel/play on any pattern, winners stack hard on whatever 1-2-crew composition the hand can produce most reliably.
